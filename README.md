...
本程序支持使用 NVIDIA GPU（通过 CUDA）加速 SHA-256 运算。

### 编译 GPU 库

需要安装 [CUDA Toolkit](https://developer.nvidia.com/cuda-downloads)：

```bash
nvcc -O3 -shared -Xcompiler -fPIC -o libgpu_miner.so sha256_miner.cu
```

### 自动选择

Python 脚本启动时自动检测：
1. 如果有 `libgpu_miner.so` + NVIDIA GPU → 使用 GPU
2. 否则如果有 `libminer.so` → 使用 CPU C 代码
3. 否则 → Python 纯代码（最慢）
...
```
  Backend: GPU [NVIDIA GeForce RTX 4090]
```

### GPU 性能对比

| GPU | SHA-256 吞吐量 | diff=33 平均出解时间 |
|-----|---------------|-------------------|
| RTX 3060 | ~2,000 MH/s | ~4 秒 |
| RTX 4090 | ~15,000 MH/s | ~0.5 秒 |
| A100 (云) | ~25,000 MH/s | ~0.3 秒 |

### 已知限制

- GPU VPS 价格较贵，需确认收益能覆盖成本
- 网络层（Cloudflare 绕过、challenge 获取）仍在 CPU 上运行
- Challenge 过期时间 ~5 分钟，GPU 通常能在过期前完成
- `libgpu_miner.so` 只在 NVIDIA GPU + CUDA 环境下编译
